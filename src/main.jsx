...
function isRural(item) {
  ...
  return RURAL_COUNTIES.has(String(item.county || '').replace(/ County$/i, '')) || /rural|critical access|mobile clinic|community health center/.test(text);
}
...
async function getStudent(email) {
  ...
  if (!clean) { alert('Enter your email to save this opportunity.'); return null; }
  const { data: existing } = await supabase.from('student_profiles').select('*').eq('email', clean).maybeSingle();
  if (existing) return existing;
  const { data, error } = await supabase.from('student_profiles').insert({ email: clean }).select().single();
  if (error) { alert('Unable to create your profile.'); return null; }
  return data;
}
...
function AppHeader({ route, navigate }) {
  ...
  return <>
    <div className="top-strip"><span>Powered by The World Savers</span><span>Student-led • Free to use • Built for impact</span></div>
    <header className="site-header">
      <button className="brand-button" onClick={() => navigate('home')}><img src="/openvol-logo.png" alt="Openvol"/><span><b>Openvol</b><small>Your healthcare journey</small></span></button>
      <nav className={open ? 'open' : ''}>{ROUTES.map(([key, label]) => <button key={key} className={route === key ? 'active' : ''} onClick={() => { navigate(key); setOpen(false); }}>{label}</button>)}</nav>
...
function SearchBar({ value, onChange, placeholder = 'Search clinics, hospitals, programs, or cities' }) {
  return <div className="search-shell"><Search size={20}/><input value={value} onChange={event => onChange(event.target.value)} placeholder={placeholder}/><button aria-label="Filters"><SlidersHorizontal size={18}/><span>Filters</span></button></div>;
}
...
function Home({ data, navigate }) {
  ...
  return <main className="phase-two-home">
    ...
    <section className="section-block"><div className="section-heading"><span>Choose your pathway</span><h2>Explore healthcare experiences built around your goals.</h2></div>
    ...
    <section className="impact-banner"><div><span>Openvol Rural Health</span><h2>Serve beyond the city limits.</h2><p>Find rural clinics, critical access hospitals, mobile care programs, and community health organizations across Georgia.</p></div><button onClick={() => navigate('rural')}>Visit Rural Health <Route size={18}/></button></section>
  </main>;
}

function Pathway({ icon, title, text, onClick, featured }) {
  return <button className={`pathway-card ${featured ? 'featured' : ''}`} onClick={onClick}><div className="pathway-icon">{icon}</div><h3>{title}</h3><p>{text}</p><span>Explore pathway <ChevronRight size={17}/></span></button>;
}

function ListingCard({ item, studentEmail, onHover }) {
  const cat = categoryOf(item);
  async function save() {
    const student = await getStudent(studentEmail); if (!student) return;
    const table = item.kind === 'clinic' ? 'saved_clinics' : 'saved_opportunities';
    const payload = item.kind === 'clinic' ? { student_id: student.id, clinic_id: item.id, application_status: 'Interested' } : { student_id: student.id, opportunity_id: item.id, application_status: 'Interested' };
  ...
  async function openUrl() {
    if (!item.url) return;
    if (item.kind === 'clinic') await trackClinicClick(item.id, item.url); else await trackOpportunityClick(item.id, item.url);
    window.open(item.url, '_blank', 'noopener,noreferrer');
  }
  return <article className="listing-card" onMouseEnter={() => onHover?.(`${item.kind}-${item.id}`)} onMouseLeave={() => onHover?.(null)}>
    <div className={`listing-logo ${cat}`}>{initials(item.organization)}</div>
    <div className="listing-main"><div className="listing-top"><div><span className={`category-pill ${cat}`}>{cat === 'clinic' ? 'Clinical' : cat === 'rural' ? 'Rural Health' : cat[0].toUpperCase() + cat.slice(1)}</span><h3>{item.title}</h3><p className="org-name">{item.organization}</p></div><button className="icon-button" onClick={save}><Bookmark size={18}/></button></div>
      <div className="meta-row"><span><MapPin size={15}/>{item.city || 'Georgia'}{item.county ? `, ${item.county} County` : ''}</span><span><Clock3 size={15}/>{item.minimum_age ? `${item.minimum_age}+` : 'Contact for age'}</span></div>
      ...
      <div className="listing-actions"><button onClick={save}>Save</button>{item.url && <button className="primary-btn small" onClick={openUrl}>View opportunity <ExternalLink size={15}/></button>}</div>
    </div>
  </article>;
}
...
function DirectoryPage({ data, mode, studentEmail, setStudentEmail }) {
  ...
  const base = mode === 'clinical' ? data.clinics : mode === 'rural' ? [...data.clinics, ...data.opportunities].filter(isRural) : data.opportunities.filter(item => categoryOf(item) === mode);
  const items = base.filter(item => `${item.title} ${item.organization} ${item.city} ${item.county} ${item.notes}`.toLowerCase().includes(query.toLowerCase()));
  ...
  return <main className="directory-page">
    <section className={`directory-hero ${mode === 'rural' ? 'rural-hero' : ''}`}><div><span className="eyebrow">{mode === 'rural' ? 'Openvol flagship pathway' : 'Explore Georgia'}</span><h1>{title}</h1><p>{subtitle}</p></div><SearchBar value={query} onChange={setQuery}/></section>
    {mode === 'rural' && <section className="rural-summary"><Stat value={items.length} label="Rural opportunities" icon={<Sprout/>}/><Stat value={new Set(items.map(item => item.county).filter(Boolean)).size} label="Counties represented" icon={<MapPin/>}/><Stat value={items.filter(item => /clinic|hospital/i.test(`${item.organization} ${item.title}`)).length} label="Clinics & hospitals" icon={<Hospital/>}/><Stat value="Georgia" label="Statewide focus" icon={<Compass/>}/></section>}
    <section className="split-layout">
      <div className="results-pane"><div className="results-toolbar"><div><b>{items.length} opportunities</b><span>Updated from the Openvol directory</span></div><label className="email-inline">Save with <input type="email" value={studentEmail} onChange={event => setStudentEmail(event.target.value)} placeholder="your email"/></label></div>

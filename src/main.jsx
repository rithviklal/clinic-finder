...
function App() {
  ...
  const [route, setRoute] = useState(APP_ROUTES.has(path) ? path : 'home');
  ...
  useEffect(() => { const pop = () => { const next = location.pathname.split('/')[1] || 'home'; setRoute(APP_ROUTES.has(next) ? next : 'home'); }; addEventListener('popstate', pop); return () => removeEventListener('popstate', pop); }, []);

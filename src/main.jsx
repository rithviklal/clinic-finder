...
const GEORGIA_CENTER = [32.75, -83.35];
...
function Home({ data, navigate }) {
  ...
  return <main className="phase-two-home">
    ...
    <section className="stats-grid phase-two-stats">
      ...
      <Stat value={data.opportunities.filter(x=>categoryOf(x)==='research').length} label="Research opportunities" icon={<Microscope/>}/>
    </section>
...
function Pathway({ icon,title,text,onClick,featured }) { return <button className={`pathway-card ${featured?'featured':''}`} onClick={onClick}><div className="pathway-icon">{icon}</div><h3>{title}</h3><p>{text}</p><span>Explore pathway <ChevronRight size={17}/></span></button>; }
...
function ListingCard({ item, studentEmail, onHover }) {
  ...
  async function openUrl() {
    ...
    if(item.kind==='clinic') await trackClinicClick(item.id,url); else await trackOpportunityClick(item.id,url);
...
function DirectoryPage({ data, mode, studentEmail, setStudentEmail }) {
  ...
  return <main className="directory-page">
    ...
    {mode==='rural'&&<section className="rural-summary"><Stat value={items.length} label="Rural opportunities" icon={<Sprout/>}/><Stat value={new Set(items.map(x=>x.county).filter(Boolean)).size} label="Counties represented" icon={<MapPin/>}/><Stat value={items.filter(x=>/clinic|hospital/i.test(`${x.organization} ${x.title}`)).length} label="Clinics & hospitals" icon={<Hospital/>}/><Stat value="Georgia" label="Statewide focus" icon={<Compass/>}/></section>}
